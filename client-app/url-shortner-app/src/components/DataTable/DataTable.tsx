...
const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
    ...
    const renderTableData = () => {
        return data.map((item) => {
            return (
                <tr key={item._id} className='border-b text-white bg-gray-600 hover:bg-white hover:text-blue-400'>
                    <td className='px-6 py-3 break-words'>
                        <Link to={item.fullUrl} target='_blank' rel='noreferrer nooopener'>{item.fullUrl}</Link>
                    </td>
                    <td className='px-6 py-3 break-words'>
                        <Link to={`${serveUrl}/shortURL/${item.shortUrl}`} target='_blank' rel='noreferrer nooopener'>{item.shortUrl}</Link>
                    </td>
    ...
    const copyToClipboard = async (url: string) => {
        try {
            await navigator.clipboard.writeText(`${serveUrl}/shortURL/${url}`)
            alert(`URL copied : ${serveUrl}/shortURL/${url}`)
        } catch (e) {
            console.log(e)
        }
    }
    ...
    const deleteUrl = async(id:string) => {
        const response = await axios.delete(`${serveUrl}/shortUrl/${id}`)
